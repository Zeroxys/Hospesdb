  ...
  connect (callback) {
    ...
    let setup = co.wrap(function * () {
      ...
      if (dbTables.indexOf('blogs') === -1) {
        ...
        yield r.db(db).table('blogs').indexWait('createdAt').run(conn)
      }
      ...
      if (dbTables.indexOf('works') === -1) {
        ...
        yield r.db(db).table('works').indexWait('createdAt').run(conn)
      }
...
/* ----------- Metodos para los trabajos grabados y listados ------------ */
  ...
  getWorks (callback) {
    ...
    let task = co.wrap(function * () {
      let conn = yield connection
      let result = yield r.db(db).table('works').orderBy({
        index: r.desc('createdAt')
      }).run(conn)
